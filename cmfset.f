...
C***  OUTER BOUNDARY CONDITION  -  FIRST ORDER
      AK=OPA(1)+OPAK(1)                          ! PHIK*OPAL(1)
      AZ=0.5*(AK+OPA(2)+OPAK(2))                 ! PHIK*OPAL(2))
      TAUZ=Z(1)-Z(2)
      DX=PP(1)/AK
      S(1)=XIMINUS+DX*DXI
      TC(1)=1./(AK*TAUZ)
...
C***  NON-BOUNDARY POINTS
      DO 1 L=2,LZ
      AK=OPA(L)+OPAK(L)                              ! PHIK*OPAL(L)
      EK=ETA(L)+ETAK(L)                              ! PHIK*ETAL(L)
      S(L)=EK/AK
      AZ=0.5*(AK+OPA(L+1)+OPAK(L+1))                 ! PHIK*OPAL(L+1))
      AZM=0.5*(AK+OPA(L-1)+OPAK(L-1))                ! PHIK*OPAL(L-1))
      TAU=0.5*(Z(L-1)-Z(L+1))
      TAUZ=Z(L)-Z(L+1)
      TAUZM=Z(L-1)-Z(L)
      DT=1./(AK*TAU)
...
C***  INNER BOUNDARY CONDITION (CORE RAYS)  -  ONLY TO FIRST ORDER
      AK=OPA(ND)+OPAK(ND)                                  ! PHIK*OPAL(ND)
...
C***  INNER BOUNDARY CONDITION (NON-CORE RAYS)  -  SECOND ORDER
    4 AK=OPA(LMAX)+OPAK(LMAX)                             ! PHIK*OPAL(LMAX)
      EK=ETA(L)+ETAK(L)                                   ! PHIK*ETAL(L)
      S(L)=EK/AK
      TAUZ=Z(LZ)
      DT=1./(AK*TAUZ)
